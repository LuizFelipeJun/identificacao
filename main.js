...
var numAleatorio = Math.floor((Math.random() * quickDrawDataset.length) + 1);
var rascunho = quickDrawDataset[numAleatorio];
document.getElementById("sketchName").innerHTML = "Esboço a ser desenhado: " + rascunho;
var canvas;
var classifier;
...
var checarTempo = "";
var guardaRascunho = "";
var respostaRascunho = "";

function updateCanvas() {
    backgroud("white");
    numAleatorio = Math.floor((Math.random() * quickDrawDataset.length) + 1);
    rascunho = quickDrawDataset[numAleatorio];
    document.getElementById("sketchName").innerHTML = "Esboço a ser desenhado: " + rascunho;
}
...
function draw() {
    ...
    sketchCheck();
    if(guardaRascunho == rascunho) {
        respostaRascunho = "set";
        pontuacao = pontuacao + 1;
        document.getElementById("score").innerHTML = "Pontuação: " + pontuacao;
    }
}
...
function gotResult(error, results) {
    if(error) {
        ...
        console.log(results);
        guardaRascunho = results[0].label;
        document.getElementById("label").innerHTML = "Seu esboço: " +  guardaRascunho.replace("_", " ");
        document.getElementById("confidence").innerHTML = "Precisão: " + Math.round(results[0].confidence * 100) + "%";
    }
}

function sketchCheck() {
    contadorDeTempo + contadorDeTempo + 1;
    document.getElementById("time").innerHTML = "Tempo: " + contadorDeTempo;
    if(contadorDeTempo > 200) {
        contadorDeTempo = 0;
        checarTempo = "Completo";
    }
    if(checarTempo == "Completo" || respostaRascunho == "set") {
        checarTempo = "";
        respostaRascunho = "";
        updateCanvas();
    }
